...
export async function GET(
  ...
  try {
    ...
    // Desencriptar datos sensibles solo para ADMIN y DOCTOR
    const isAuthorized = session.user.role === 'ADMIN' || session.user.role === 'DOCTOR'
    
    const decryptedPatient = isAuthorized 
      ? decryptObject(patient, ['phone', 'address'])
      : patient
    
    // Desencriptar datos sensibles de las consultas (solo para roles autorizados)
