...
export default function BillingPageUpdated() {
  ...
  const handleCreatePatient = async (e: React.FormEvent) => {
    ...
    if (!newPatientData.name.trim() || !newPatientData.birthDate || !newPatientData.gender) {
      toast.error("Debes completar los campos obligatorios (Nombre, Fecha de Nacimiento, Género)")
      return
    }

    // Calcular edad para validar cédula
    const age = calculateAge(newPatientData.birthDate)
    const isMinor = age < 18

    // Si es menor de edad, la cédula no es obligatoria
    if (!isMinor && !newPatientData.cedula.trim()) {
    ...
    try {
      ...
      const patientPayload = {
        ...
        birthDate: newPatientData.birthDate, // Enviar como string directamente
        gender: newPatientData.gender,
        phone: newPatientData.phone.trim() || null,
        address: newPatientData.address.trim() || null,
        nationality: newPatientData.nationality.trim() || null,
        cedula: newPatientData.cedula.trim() || null
      }
      
      console.log("Enviando datos del paciente:", patientPayload)
      ...
      if (response.ok) {
        ...
        // Si estamos en el diálogo de crear factura, seleccionar automáticamente el nuevo paciente
        ...
        toast.error(errorData.error || `Error al crear el paciente (${response.status})`)
      }
    } catch (error) {
          ...
          <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreatePatient}>
              <div className="space-y-4 pt-4">
                {/* Nombre Completo */}
                <div className="grid grid-cols-4 items-start gap-4">
                  <Label htmlFor="patient-name" className="text-right mt-2">
                    Nombre Completo *
                  </Label>
                  <div className="col-span-3">
                    <Input
                ...
                {/* Fecha de Nacimiento */}
                <div className="grid grid-cols-4 items-start gap-4">
                  <Label htmlFor="patient-birthDate" className="text-right mt-2">
                    Fecha de Nacimiento *
                  </Label>
                  <div className="col-span-3">
                    <Input
                ...
                {/* Género */}
                <div className="grid grid-cols-4 items-start gap-4">
                  <Label htmlFor="patient-gender" className="text-right mt-2">
                    Género *
                  </Label>
                  <div className="col-span-3">
                    <Select
                ...
                {/* Teléfono */}
                <div className="grid grid-cols-4 items-start gap-4">
                  <Label htmlFor="patient-phone" className="text-right mt-2">
                    Teléfono
                  </Label>
                  <div className="col-span-3">
                    <Input
                ...
                {/* Dirección */}
                <div className="grid grid-cols-4 items-start gap-4">
                  <Label htmlFor="patient-address" className="text-right mt-2">
                    Dirección
                  </Label>
                  <div className="col-span-3">
                    <Input
                ...
                {/* Nacionalidad */}
                <div className="grid grid-cols-4 items-start gap-4">
                  <Label htmlFor="patient-nationality" className="text-right mt-2">
                    Nacionalidad
                  </Label>
                  <div className="col-span-3">
                    <Input
                      ...
                      onChange={(e) => setNewPatientData(prev => ({ ...prev, nationality: e.target.value }))}
                    />
                  </div>
                </div>
                ...
                {/* Cédula */}
                <div className="grid grid-cols-4 items-start gap-4">
                  <Label htmlFor="patient-cedula" className="text-right mt-2">
                    Cédula {newPatientData.birthDate && calculateAge(newPatientData.birthDate) >= 18 ? '*' : ''}
                  </Label>
                  <div className="col-span-3">
                    <Input
                      id="patient-cedula"
                      placeholder={
                        newPatientData.birthDate && calculateAge(newPatientData.birthDate) < 18 
                          ? "Opcional para menores de edad" 
                          : "Ej: 12345678901"
                      }
                      value={newPatientData.cedula}
                      onChange={(e) => setNewPatientData(prev => ({ ...prev, cedula: e.target.value }))}
                      required={newPatientData.birthDate ? calculateAge(newPatientData.birthDate) >= 18 : false}
                    />
                    {newPatientData.birthDate && calculateAge(newPatientData.birthDate) < 18 && (
              ...
              <DialogFooter>
                <Button 
                  type="button" 
                  variant="outline" 
                  onClick={() => setIsCreatePatientDialogOpen(false)}
                ...
                <Button 
                  ...
                  disabled={
                    ...
                    (newPatientData.birthDate && calculateAge(newPatientData.birthDate) >= 18 && !newPatientData.cedula.trim())
                  }
                >
                  {isCreatingPatient ? (
                    <>
